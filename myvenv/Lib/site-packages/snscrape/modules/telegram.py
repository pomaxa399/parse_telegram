...
class TelegramPost(snscrape.base.Item):
	...
	video_preview: typing.Optional[str] = None
	linkpreview: typing.Optional[str] = None
	views: typing.Optional[str] = None
	...
	def _soup_to_items(self, soup, pageUrl, onlyUsername = False):
		...
		for post in reversed(posts):
			...
			if (message := post.find('div', class_ = 'tgme_widget_message_text')):
				content = message.prettify()
				title = post.find('div', {'class': 'tgme_widget_message_author'}).text
				video_preview = None
				photo_wrap = post.select_one('i.tgme_widget_message_video_thumb')
				if photo_wrap:
					style = photo_wrap.get('style')
					if style:
						match = re.search(r"background-image:url\('(.+)'\)", style)
						if match:
							video_preview = match.group(1)
				
				views = None
				views_prev = post.find('span', {'class': 'tgme_widget_message_views'}).text.split()[0]
				if 'K' in views_prev:
					views = str(int(float(views_prev.replace('K', '')) * 1000))
				else:
					views = str(views_prev)

				# video_link = None
				# video_prev = post.find('div', {'class': 'tgme_widget_message_video_wrap'})
				# # video_link = video_prev.find('video')['src']
				# if video_prev and (video := video_prev.find('video')):
				...
				linkpreview = None
				photo_wrap = post.select_one('i.link_preview_image')
				if photo_wrap:
					style = photo_wrap.get('style')
					if style:
						match = re.search(r"background-image:url\('(.+)'\)", style)
						if match:
							linkpreview = match.group(1)
				# outlinks = []
			...
			yield TelegramPost(chat_id_or_url = url, date = date, content = content, #outlinks = outlinks, #linkPreview = linkPreview,
		      					image_url=image_link, title=title,
		      					video_preview=video_preview, linkpreview=linkpreview, views=views)
